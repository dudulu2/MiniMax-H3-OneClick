...
function Invoke-PipWithFallback {
    ...
    $official = if ($NeedsTorchIndex) {
        "$PackageArguments --index-url https://download.pytorch.org/whl/cu130"
    } else {
        "$PackageArguments --index-url https://pypi.org/simple --extra-index-url https://download.pytorch.org/whl/cu130"
    }
    $mirror = if ($NeedsTorchIndex) {
        "$PackageArguments --index-url https://mirrors.aliyun.com/pytorch-wheels/cu130"
    } else {
        "$PackageArguments --index-url https://pypi.tuna.tsinghua.edu.cn/simple --extra-index-url https://mirrors.aliyun.com/pytorch-wheels/cu130"
    }
...
function Install-PythonRuntime {
    ...
    $null = Invoke-SimpleDownload "Python 3.13.9" @(
...
function Install-ComfyEnvironment {
    ...
    Set-Stage "Installing PyTorch 2.12.0 CUDA 13.0" -1
    Invoke-PipWithFallback $venvPython "install torch==2.12.0+cu130 torchvision==0.27.0+cu130 torchaudio==2.11.0+cu130" -NeedsTorchIndex

    Set-Stage "Installing SageAttention and Triton" -1
    Install-SageAttentionRuntime -Python $venvPython -InstallRoot $InstallRoot

    $constraints = Join-Path $InstallRoot "runtime\constraints-cu130.txt"
    @"
torch==2.12.0+cu130
...
"@ | Set-Content -LiteralPath $constraints -Encoding ASCII
    ...
    $verifyCode = "import torch,torchvision,torchaudio; assert torch.__version__=='2.12.0+cu130'; assert torchvision.__version__=='0.27.0+cu130'; assert torchaudio.__version__=='2.11.0+cu130'; assert torch.cuda.is_available(); print(torch.cuda.get_device_name(0)); print('DynamicVRAM prerequisite ready')"
