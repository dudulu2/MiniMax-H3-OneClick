function Install-PythonRuntime {
    ...
    if (Test-Path -LiteralPath $python) {
        ...
        if ($version -eq "3.13") {
            Add-Log "Existing private Python 3.13 runtime found."
            return $python
        }
        Add-Log "Existing private Python $version runtime is outdated; upgrading to Python 3.13. The old virtual environment will be rebuilt." "WARN"
        $venvRoot = Join-Path $InstallRoot "runtime\venv"
        if (Test-Path -LiteralPath $venvRoot) {
            Remove-Item -LiteralPath $venvRoot -Recurse -Force
            Add-Log "Removed the old Python virtual environment for the 3.13 upgrade."
        }
    ...
    $installer = Join-Path $cache "python-3.13.9-amd64.exe"
    Set-Stage "Downloading Python 3.13 runtime" -1
    $mirrorFirst = Test-ChinaMirrorPriority
    $pythonUrls = if ($mirrorFirst) {
        @(
            "https://registry.npmmirror.com/-/binary/python/3.13.9/python-3.13.9-amd64.exe",
    ...
    $null = Invoke-SimpleDownload "Python 3.13.9" $pythonUrls $installer
...
function Install-SageAttentionRuntime {
    ...
    $tritonWheelName = "triton_windows-3.7.1.post27-cp313-cp313-win_amd64.whl"
    $sageWheelName = "sageattention-2.2.0+cu130torch2.10.0andhigher.post6-cp310-abi3-win_amd64.whl"

    $sageState = (& $Python -c "import importlib.metadata as m`ntry:`n print(m.version('sageattention'))`nexcept Exception:`n print('')" 2>$null | Select-Object -Last 1)
    if ($sageState -like "2.2.0*") {
        Add-Log "SageAttention $sageState is already installed; skipping SageAttention/Triton setup."
        return
    }

    $tritonWheel = Join-Path $wheelsDir $tritonWheelName
    if (Test-Path -LiteralPath $tritonWheel) {
        Set-Stage "Installing Triton 3.7.1 from bundled wheel" -1
        $null = Invoke-ProcessChecked $Python ("-m pip install `"{0}`" --disable-pip-version-check" -f $tritonWheel) $script:InstallerRoot
    } else {
        Set-Stage "Installing Triton 3.7.1 from Python package sources" -1
        Invoke-BasePyPiWithFallback -Python $Python -PackageArguments "install triton_windows==3.7.1.post27"
    }
    ...
    Set-Stage "Installing SageAttention 2.2 from bundled wheel" -1
    $null = Invoke-ProcessChecked $Python ("-m pip install `"{0}`" --disable-pip-version-check" -f $sageWheel) $script:InstallerRoot

    Set-Stage "Verifying SageAttention runtime" -1
    $verifySage = "import triton`nprint('triton', triton.__version__)`nprint('sageattention', __import__('importlib.metadata', fromlist=['version']).version('sageattention'))"
    $null = Invoke-ProcessChecked $Python ("-c `"{0}`"" -f $verifySage) $script:InstallerRoot
}
...
function Install-ComfyEnvironment {
    ...
    Set-Stage "Installing SageAttention and Triton" -1
...
function Invoke-Install {
    ...
    try {
        ...
        Add-Log "SageAttention 2.2 and Triton 3.7.1 will be installed for H3 acceleration. No SeedVR2, xformers, FlashAttention, or other custom compute nodes are included."
