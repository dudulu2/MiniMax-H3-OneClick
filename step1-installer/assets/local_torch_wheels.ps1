...
function Get-TorchWheelUrls {
    ...
    # The "+" in +cu130 must be percent-encoded ("%2B") or the official
    # download.pytorch.org source rejects the request. Both sources accept %2B.
    $encodedName = $FileName.Replace("+", "%2B")
    $official = ([string]$Runtime.index_url).TrimEnd('/') + "/" + $encodedName
    $mirror = $null
    if ($Runtime.mirror_index_url) {
        $mirror = ([string]$Runtime.mirror_index_url).TrimEnd('/') + "/" + $encodedName
    }
...
function Install-SelectedTorchRuntime {
    ...
    $cacheDir = Join-Path $InstallRoot ("downloads\torch-wheels\cu{0}-py313" -f ([string]$Runtime.cuda_version).Replace('.', ''))
    New-Item -ItemType Directory -Force -Path $cacheDir | Out-Null

    $wheels = @{}
    foreach ($package in $packageSpecs) {
        $fileName = "{0}-{1}-cp313-cp313-win_amd64.whl" -f $package.Name, $package.Version
